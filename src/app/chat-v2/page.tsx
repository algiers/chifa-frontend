...
function ChatV2Content() {
  ...
  useEffect(() => {
    ...
    const loadConversation = async (conversationId: string) => {
      ...
      setChatError(null);
      try {
        // Utiliser les nouvelles tables conversations/messages
        ...
        if (data && data.length > 0) {
          const fetchedMessages = data.map((msg: any) => ({
            id: msg.id,
            role: msg.role,
            content: typeof msg.content === 'string' ? msg.content : msg.content_text || '',
            sqlQuery: msg.content?.sql_query || null,
            sqlResults: msg.content?.sql_results || null,
            errorMessage: null,
  ...
  useEffect(() => {
    ...
    if (isAuthReady && !user) {
      router.replace('/login?redirectTo=/chat-v2');
    }
  }, [isAuthReady, isAdmin, user, router]);

  useEffect(() => {
    if (!user) {
      router.push('/login?redirectTo=/chat-v2');
      return;
    }
      ...
      <div className="min-h-screen flex items-center justify-center bg-muted/50 px-4">
        <Card className="border-0 shadow-xl max-w-md w-full">
          <CardHeader className="text-center">
            ...
            <CardDescription>Veuillez vous connecter pour accéder au chat</CardDescription>
          </CardHeader>
          <CardContent className="text-center">
            <Button asChild className="w-full bg-green-600 hover:bg-green-700">
              <a href="/login?redirectTo=/chat-v2">Se connecter</a>
            </Button>
          </CardContent>
        </Card>
      </div>
...
// Wrap ChatV2Content with Suspense for useSearchParams
